...
async function getContributionData(
  ...
  try {
    ...
    return allDays; // No longer padding here, will be handled later
  } catch (error) {
...
const ContributionGraph = async () => {
          ...
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="text-gray-600">
                Please sign in to view your GitHub contribution history
              </p>
            </div>
          </div>
        </div>
  ...
  try {
    ...
    if (!user) {
      // This should ideally not happen if the webhook is working correctly
          ...
          <div className="relative bg-white/80 backdrop-blur-sm p-8 rounded-2xl shadow-xl border border-gray-200/50 mt-8">
            ...
            <div className="flex items-center justify-center py-12">
              <div className="text-center">
                ...
                <p className="text-gray-600">
                  Please reconnect your GitHub account to view contribution data
                </p>
              </div>
            </div>
          </div>
    ...
    let contributionDays: ContributionDay[] = user.contributions.map(
      (c) => ({
        contributionCount: c.count,
        date: formatDateToISO(c.date),
        weekday: c.date.getDay(),
        // A default color, will be updated if new data is fetched
        color: '#ebedf0',
      })
    );

    const fromDateForApi = user.contributionsLastTracked
      ? user.contributionsLastTracked
      : from;

    const newContributionDays = await getContributionData(
      githubToken,
      fromDateForApi.toISOString(),
      to.toISOString()
    );

    if (newContributionDays) {
      const existingContributionDates = new Set(
        contributionDays.map((d) => d.date)
      );
      const mergedContributions = [...contributionDays];

      newContributionDays.forEach((newDay) => {
        if (!existingContributionDates.has(newDay.date)) {
          mergedContributions.push(newDay);
        } else {
          // Update existing day with new data
          const existingDayIndex = mergedContributions.findIndex(
            (d) => d.date === newDay.date
          );
          if (existingDayIndex !== -1) {
            mergedContributions[existingDayIndex] = newDay;
          }
        }
      });
      ...
      const filteredContributions = mergedContributions.filter(
        (day) => new Date(day.date) >= sixtyDaysAgo
      );

      contributionDays = padContributionData(filteredContributions);

      // Update database in a transaction
      await prisma.$transaction([
        prisma.contribution.deleteMany({ where: { userId: user.id } }),
        ...
        <div className="relative bg-white/90 dark:bg-background dark:border-2 dark:border-gray-600 backdrop-blur-sm p-8 rounded-2xl shadow-2xl border border-white/20 mt-8 overflow-hidden">
          ...
          <div className="relative flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              <div>
                ...
                <p className="text-md pl-1 font-semibold mt-1">
                  <span className="text-green-600">{totalContributions}</span>{' '}
                  contributions over{' '}
                  <span className="text-blue-600">{activeDays}</span> active
                  days
                </p>
              </div>
            </div>
          ...
          <div className="relative">
                        ...
                        : Math.min(
                            ...
                            Math.ceil(
                              (day.contributionCount /
                                Math.max(maxContributions, 1)) *
                                4
                            )
                          );
                    const shadowIntensity =
                      intensity > 0 ? 'shadow-sm' : 'shadow-sm';
            ...
            <div className="flex items-center justify-center mt-4 px-2">
              <span className="text-sm text-gray-600 font-medium pr-4 dark:text-primary">
                Less
              </span>
              ...
              <span className="text-sm text-gray-600 font-medium pl-4 dark:text-primary">
                More
              </span>
            </div>
          </div>
        ...
        <div className="relative bg-white/80 backdrop-blur-sm p-8 rounded-2xl shadow-xl border border-gray-200/50 mt-8">
          ...
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="text-gray-600">
                An error occurred while loading contribution data
              </p>
            </div>
          </div>
        </div>
